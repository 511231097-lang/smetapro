...
const setupApiMock = async (page: Page, options: MockOptions = {}) => {
  const defaultWorkspaceId =
    options.workspaces?.[0]?.id ?? mockWorkspace.id;
  const state = {
    ...
    adminWorkspaces:
      options.adminWorkspaces ?? (options.workspaces ?? [mockWorkspace]),
    projectsByWorkspace:
      options.projectsByWorkspace ?? {
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (url.pathname.startsWith("/api/v1/projects/")) {
      ...
      if (method === "DELETE") {
        ...
        const workspaceProjects =
          state.projectsByWorkspace[project.workspace_id] ?? [];
        state.projectsByWorkspace[project.workspace_id] = workspaceProjects.filter(
